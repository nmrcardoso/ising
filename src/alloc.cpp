...
namespace ising {
  ...
  static void print_trace (void) {
    ...
    printfIsing("Obtained %zd stack frames.\n", size);
    for (size_t i=0; i<size; i++) printfIsing("%s\n", strings[i]);
    free(strings);
  }

  static void print_alloc_header()
  {
    printfIsing("Type        Pointer          Size             Location\n");
    printfIsing("----------------------------------------------------------\n");
  }
  ...
  bool use_managed_memory()
  {
    ...
    if (!init) {
      ...
      if (enable_managed_memory && strcmp(enable_managed_memory, "1") == 0) {
        ...
        if (deviceProp.major < 6) printfIsing("Warning: Using managed memory on pre-Pascal architecture is limited\n");
      }
  ...
  static void *aligned_malloc(MemAlloc &a, size_t size)
  {
    ...
    if (!ptr || align != 0) {
#endif
      printfIsing("ERROR: Failed to allocate aligned host memory of size %zu (%s:%d in %s())\n", size, a.file.c_str(), a.line, a.func.c_str());
      errorIsing("Aborting\n");
    }
    return ptr;
  }
  ...
  void *dev_malloc_(const char *func, const char *file, int line, size_t size)
  {
    ...
    if (err != cudaSuccess) {
      printfIsing("ERROR: Failed to allocate device memory of size %zu (%s:%d in %s())\n", size, file, line, func);
      errorIsing("Aborting\n");
    }
...
//#ifdef HOST_DEBUG
    ...
    if (err != cudaSuccess) {
      printfIsing("ERROR: Failed to set device memory of size %zu (%s:%d in %s())\n", size, file, line, func);
      errorIsing("Aborting\n");
    }
//#endif
...
//#endif
  ...
  void *safe_malloc_(const char *func, const char *file, int line, size_t size)
  {
    ...
    if (!ptr) {
      printfIsing("ERROR: Failed to allocate host memory of size %zu (%s:%d in %s())\n", size, file, line, func);
      errorIsing("Aborting\n");
    }
...
//#endif
  ...
  void *pinned_malloc_(const char *func, const char *file, int line, size_t size)
  {
    ...
    if (err != cudaSuccess) {
      printfIsing("ERROR: Failed to register pinned memory of size %zu (%s:%d in %s())\n", size, file, line, func);
      errorIsing("Aborting\n");
    }
...
#define HOST_ALLOC // this needs to be set presently on P9
  ...
  void *mapped_malloc_(const char *func, const char *file, int line, size_t size)
  {
    ...
    if (err != cudaSuccess) {
      printfIsing("ERROR: cudaHostAlloc failed of size %zu (%s:%d in %s())\n", size, file, line, func);
      errorIsing("Aborting\n");
    }
#else
    void *ptr = aligned_malloc(a, size);
    ...
    if (err != cudaSuccess) {
      printfIsing("ERROR: Failed to register host-mapped memory of size %zu (%s:%d in %s())\n", size, file, line, func);
      errorIsing("Aborting\n");
    }
#endif
  ...
  void *managed_malloc_(const char *func, const char *file, int line, size_t size)
  {
    ...
    if (err != cudaSuccess) {
      errorIsing("Failed to allocate managed memory of size %zu (%s:%d in %s())\n", size, file, line, func);
    }
...
//#ifdef HOST_DEBUG
    cudaError_t err1 = cudaMemset(ptr, 0, size);
    if (err1 != cudaSuccess) {
      printfIsing("ERROR: Failed to set managed memory of size %zu (%s:%d in %s())\n", size, file, line, func);
      errorIsing("Aborting\n");
    }
#endif
    return ptr;
  ...
  {
    if (!ptr) { errorIsing("Attempt to free NULL managed pointer (%s:%d in %s())\n", file, line, func); }
    if (!alloc[MANAGED_PTR].count(ptr)) {
      errorIsing("Attempt to free invalid managed pointer (%s:%d in %s())\n", file, line, func);
    }
    cudaError_t err = cudaFree(ptr);
    if (err != cudaSuccess) { errorIsing("Failed to free device memory (%s:%d in %s())\n", file, line, func); }
  ...
  void dev_free_(const char *func, const char *file, int line, void *ptr)
  {
    ...
    if (!ptr) {
      printfIsing("ERROR: Attempt to free NULL device pointer (%s:%d in %s())\n", file, line, func);
      errorIsing("Aborting\n");
    }
    if (!alloc[DEVICE_PTR].count(ptr)) {
      printfIsing("ERROR: Attempt to free invalid device pointer (%s:%d in %s())\n", file, line, func);
      errorIsing("Aborting\n");
    }
    cudaError_t err = cudaFree(ptr);
    if (err != cudaSuccess) {
      printfIsing("ERROR: Failed to free device memory (%s:%d in %s())\n", file, line, func);
      errorIsing("Aborting\n");
    }
  ...
  {
    if (!ptr) {
      printfIsing("ERROR: Attempt to free NULL host pointer (%s:%d in %s())\n", file, line, func);
      errorIsing("Aborting\n");
    }
    if (alloc[HOST_PTR].count(ptr)) {
      ...
      if (err != cudaSuccess) {
	printfIsing("ERROR: Failed to unregister pinned memory (%s:%d in %s())\n", file, line, func);
	errorIsing("Aborting\n");
      }
      ...
      if (err != cudaSuccess) {
	printfIsing("ERROR: Failed to free host memory (%s:%d in %s())\n", file, line, func);
	errorIsing("Aborting\n");
      }
#else
      cudaError_t err = cudaHostUnregister(ptr);
      if (err != cudaSuccess) {
	printfIsing("ERROR: Failed to unregister host-mapped memory (%s:%d in %s())\n", file, line, func);
	errorIsing("Aborting\n");
      }
      free(ptr);
#endif
      ...
      printfIsing("ERROR: Attempt to free invalid host pointer (%s:%d in %s())\n", file, line, func);
      print_trace();
      errorIsing("Aborting\n");
    }
  }
  ...
  void assertAllMemFree()
  {
    if (!alloc[DEVICE_PTR].empty() || !alloc[DEVICE_PINNED_PTR].empty() || !alloc[HOST_PTR].empty() || !alloc[PINNED_PTR].empty() || !alloc[MAPPED_PTR].empty() || !alloc[MANAGED_PTR].empty()) {
  	...
  	else printfIsing("All Memory Free!\n");
  }
  ...
  FieldLocation get_pointer_location(const void *ptr) {
    ...
    if (error != CUDA_SUCCESS) {
      ...
      errorIsing("cuPointerGetAttributes failed with error %s", string);
    }
    ...
    default:
      errorIsing("Unknown memory type %d", mem_type);
  ...
  void *get_mapped_device_pointer_(const char *func, const char *file, int line, const void *host)
  {
    ...
    if (error != cudaSuccess) {
      errorIsing("cudaHostGetDevicePointer failed with error %s (%s:%d in %s()",
...
void FreeAllMemory(){
    ...
    if (!alloc[DEVICE_PINNED_PTR].empty() ) {
      printfIsing("Releasing DEVICE_PINNED internal memory allocations not freed by user:\n");
      print_alloc_header();
      FreeMemoryType(DEVICE_PINNED_PTR);
      printfIsing("\n");
    }
    if (!alloc[PINNED_PTR].empty() ) {
      printfIsing("Releasing PINNED internal memory allocations not freed by user:\n");
      print_alloc_header();
      FreeMemoryType(PINNED_PTR);
      printfIsing("\n");
    }
    if (!alloc[MAPPED_PTR].empty() ) {
      printfIsing("Releasing MAPPED internal memory allocations not freed by user:\n");
      print_alloc_header();
      FreeMemoryType(MAPPED_PTR);
      printfIsing("\n");
    }
    if (!alloc[HOST_PTR].empty() ) {
      printfIsing("Releasing HOST internal memory allocations not freed by user:\n");
      print_alloc_header();
