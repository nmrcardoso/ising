...
#define ISING_VERSION_MAJOR     0
#define ISING_VERSION_MINOR     1
#define ISING_VERSION_SUBMINOR  0

/**
 * @def   ISING_VERSION
 * @brief This macro is deprecated.  Use ISING_VERSION_MAJOR, etc., instead.
 */
#define ISING_VERSION ((ISING_VERSION_MAJOR<<16) | (ISING_VERSION_MINOR<<8) | ISING_VERSION_SUBMINOR)
...
//CODE FROM QUDA LIBRARY WITH A FEW MODIFICATIONS
  ...
  static const std::string Ising_hash = ISING_HASH; // defined in lib/Makefile
...
#define STR_(x) #x
#define STR(x) STR_(x)
  static const std::string CULQCD_version = STR(ISING_VERSION_MAJOR) "." STR(ISING_VERSION_MINOR) "." STR(ISING_VERSION_SUBMINOR);
#undef STR
#undef STR_
  ...
  void loadTuneCache(Verbosity verbosity)
  {
    ...
    path = getenv("ISING_RESOURCE_PATH");
    if (!path) {
      printfIsing("warning: Environment variable ISING_RESOURCE_PATH is not set.\n");
      //printfIsing("warning: Caching of tuned parameters will be disabled.");
	  resource_path=GetCurrentWorkingDir();
      printfIsing("warning: using default file in current directory: %s\n", resource_path.c_str());
      //return;
    } else if (stat(path, &pstat) || !S_ISDIR(pstat.st_mode)) {
      printfIsing("warning: The path \"%s\" specified by ISING_RESOURCE_PATH does not exist or is not a directory.\n", path); 
      //printfIsing("warning: Caching of tuned parameters will be disabled.");
	  resource_path=GetCurrentWorkingDir();
      printfIsing("warning: using default file in current directory: %s\n", resource_path.c_str());
      //return;
    ...
    if (!cache_file.good()) errorIsing("Bad format in %s", cache_path.c_str());
    getline(cache_file, line);
    ls.str(line);
    ls >> token;
    if (token.compare("tunecache")) errorIsing("Bad format in %s", cache_path.c_str());
    ls >> token;
    if (token.compare(CULQCD_version)) errorIsing("Cache file %s does not match current CULQCD version. \nPlease delete this file or set the ISING_RESOURCE_PATH environment variable to point to a new path.", cache_path.c_str());
    ls >> token;
    if (token.compare(Ising_hash)) errorIsing("Cache file %s does not match current CULQCD build. \nPlease delete this file or set the ISING_RESOURCE_PATH environment variable to point to a new path.", cache_path.c_str());
    ...
    if (verbosity >= SUMMARIZE) {
      printfIsing("Loaded %d sets of cached parameters from %s\n", static_cast<int>(initial_cache_size), cache_path.c_str());
    }
      

      } else {
    printfIsing("warning: Cache file not found.  All kernels will be re-tuned (if tuning is enabled).");
      }
...
//#endif
  ...
  void saveTuneCache(Verbosity verbosity)
  {
    ...
    if (mynode() == 0)
      ...
      if (lock_handle == -1) {
    printfIsing("warning: Unable to lock cache file.  Tuned launch parameters will not be cached to disk.  "
    ...
    "this lock file shouldn't be here and is safe to delete.";
      ...
      if (stat == -1) printfIsing("warning: Unable to write to lock file for some bizarre reason");

      cache_path = resource_path + "/tunecache.csv";
      cache_file.open(cache_path.c_str());
    
      if (verbosity >= SUMMARIZE) {
    printfIsing("Saving %d sets of cached parameters to %s\n", static_cast<int>(tunecache.size()), cache_path.c_str());
      }
    
      time(&now);
      cache_file << "tunecache\t" << CULQCD_version << "\t" << Ising_hash << "\t# Last updated " << ctime(&now) << std::endl;
...
TuneParam& tuneLaunch(Tunable &tunable, TuneMode enabled, Verbosity verbosity){
    ...
    if (enabled == TUNE_NO) {
      ...
      if (verbosity >= DEBUG_VERBOSE) printfIsing("PreTune %s\n", key.name);
      tunable.preTune();
      ...
      if (verbosity >= DEBUG_VERBOSE) {
        printfIsing("Tuning %s with %s at vol=%s\n", key.name, key.aux, key.volume);
      }
      ...
      while (tuning) {
        ...
        for (int i=0; i<tunable.tuningIter(); i++) {
          if (verbosity >= DEBUG_VERBOSE) {
            printfIsing("About to call tunable.apply\n");
          }
        ...
        if (error != cudaSuccess) errorIsing("Failed to clear error state %s\n", cudaGetErrorString(error));
        }
      ...
      if (best_time == FLT_MAX) {
        errorIsing("Auto-tuning failed for %s with %s at vol=%s\n", key.name, key.aux, key.volume);
      }
      if (verbosity >= VERBOSE) {
        printfIsing("Tuned %s giving %s for %s with %s\n", tunable.paramString(best_param).c_str(),
      ...
      if (verbosity >= DEBUG_VERBOSE) printfIsing("PostTune %s\n", key.name);
      tunable.postTune();
      param = best_param;
      ...
      errorIsing("Unexpected call to tuneLaunch() in %s::apply()", typeid(tunable).name());
    }
