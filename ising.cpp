...
int main(){
	...
	int dirs = 2; 
	int ls = 32; //The number of points in each direction must be an even number!!!!!!!!!
	int Nx=ls;
	int Ny=ls;
	int Nz=ls;
	int Nt=ls;
	...
	SetupLatticeParameters(Nx, Ny, Nz, Nt, dirs, beta, jconst, imetrop);
...
if(temperature> 2 && temperature < 3) step = 0.01;
	...
	for(PARAMS::iter = 1; PARAMS::iter <= maxIter; ++PARAMS::iter){
		...
		ComputeAVG(lattice, &avgspin, &energy);	
		if(PARAMS::iter >= mininter) mag.push_back(abs(avgspin));
	...
	cout << "###################################################################################" << endl;
	cout << temperature << "\t#wopt: " << wopt << "\t#configs: " << nconfigs << "\tMean: " << Mmean.x << " ± " << Mmean.y << "  ::suceptibility: " << sus.x << " ± " << sus.y << endl;
	
	
	
	fileout1 << grid << Jconst() << '\t' << temperature << '\t' << wopt << '\t' << nconfigs << '\t' << Mmean.x << '\t' << Mmean.y << '\t' << sus.x << '\t' << sus.y << '\t' << sus.x*Volume() << '\t' << sus.y*Volume() << endl;
	...
	gp.sendLine("reset;");
	gp.sendLine("set terminal x11 size 1000,600 enhanced font 'Verdana,10' persist");
	gp.sendLine("set multiplot layout 2,3 rowsfirst");
	gp.sendLine("unset label");
	gp.sendLine("set grid");
	gp.sendLine("set mxtics 5");
	gp.sendLine("set mytics 5");
